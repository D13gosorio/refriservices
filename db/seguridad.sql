...
--
--  Se ejecuta en el SQL Editor de Supabase, de arriba a abajo. Es repetible
--  sin efectos, con una sola excepción: el índice único de correos del PASO 2
--  falla si ya hay duplicados; el propio paso explica cómo detectarlos.
--
--  QUÉ RESUELVE
--  ------------
...
-- ----------------------------------------------------------------------------
-- PASO 0. Comprobación previa (imprescindible antes de seguir)
-- ----------------------------------------------------------------------------
...
--
-- Resultado a revisar antes de continuar:
...
-- Solo se continúa si el usuario de DB_USER figura como `tableowner` de las
-- tablas, o si `es_superusuario` / `ignora_rls` da true. En caso contrario hay
-- que traspasarle la propiedad primero, tabla por tabla:
--
...
-- ----------------------------------------------------------------------------
-- PASO 2. Índices que evitan recorridos completos de tabla
-- ----------------------------------------------------------------------------
...
--
-- Es el único paso que puede fallar, y falla si ya existen correos
-- duplicados. Esta consulta no debe devolver ninguna fila:
--
...
--     HAVING COUNT(*) > 1;
--
-- Si devuelve filas, hay que dejar una sola cuenta por correo antes de
-- continuar. Alternativa provisional: quitar UNIQUE y el índice seguirá
-- acelerando el login, aunque sin impedir duplicados.
CREATE UNIQUE INDEX IF NOT EXISTS idx_usuarios_email
    ON public.usuarios (LOWER(email));


-- ----------------------------------------------------------------------------
-- PASO 3. Row Level Security en todas las tablas
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- PASO 4. Retirada de permisos a los roles de la API REST
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- Las siete tablas deben aparecer con rls_activada = true.
...
-- Y esta no debe devolver ninguna fila, es decir, ningún permiso suelto:
...
-- ----------------------------------------------------------------------------
-- PASO 6. Pendiente en el panel de Supabase
-- ----------------------------------------------------------------------------
--  1. Settings → Database: cambiar la contraseña de la base de datos y
--     actualizar DB_PASS en Vercel. La anterior estuvo en un repositorio
--     público, así que se da por comprometida.
--
--  2. Cambiar las contraseñas de las cuentas de prueba, que también
--     estuvieron publicadas. El hash se genera con PHP:
--
...
--
--     La contraseña en claro nunca va en esta consulta.
--
--  3. Settings → API: si no se va a usar la API REST, restringir el acceso
--     o rotar las claves `anon` y `service_role`.
-- ============================================================================
